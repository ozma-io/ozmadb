module FunWithFlags.FunUtils.Expr
...
let rec private assignmentToMemberBinding (obj : Var) (bindings : MemberBinding list) : Expr -> (MemberBinding list) option = function
    | Patterns.Sequential(next, Patterns.PropertySet(Some (Patterns.Var obj'), propertyInfo, [], expr)) when obj = obj' ->
        let bind = Expression.Bind(propertyInfo :> MemberInfo, LeafExpressionConverter.QuotationToExpression expr) :> MemberBinding
        assignmentToMemberBinding obj (bind :: bindings) next
    | Patterns.Value(null, t) when t = typeof<unit> ->
        Some bindings
    | e ->
        failwithf "Unexpected expression: %O" e

let toMemberInit (expr : Expr<'arg -> 'o>) : Expression<Func<'arg, 'o>> =
    match expr with
    | Patterns.Lambda(v, Patterns.Let(obj, Patterns.NewObject (constr, args), Patterns.Sequential(assignments, Patterns.Var obj'))) when obj = obj' ->
        match assignmentToMemberBinding obj [] assignments with
        | Some members ->
            let newExpr = Expression.New(constr, Seq.map LeafExpressionConverter.QuotationToExpression args)
        ...
        | None ->
            failwithf "Failed to convert assignments to member init: %O" assignments
    | e ->
        failwithf "Unexpected expression: %O" e
