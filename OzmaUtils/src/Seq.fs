...
let first (s: seq<'a>) : 'a option =
    ...
    if i.MoveNext() then Some i.Current else None
...
let inline fold1 (func: 'a -> 'a -> 'a) (s: seq<'a>) : 'a = Seq.fold func (Seq.head s) (Seq.tail s)
...
let inline foldOption (func: 'acc -> 'a -> 'acc option) (init: 'acc) (vals: seq<'a>) : 'acc option =
    ...
    if failed then None else Some acc
...
let inline foldOptionTask (func: 'acc -> 'a -> Task<'acc option>) (init: 'acc) (vals: seq<'a>) : Task<'acc option> =
    task {
        ...
        if failed then return None else return Some acc
    }

let inline foldResultTask
    (func: 'acc -> 'a -> Task<Result<'acc, 'e>>)
    (init: 'acc)
    (vals: seq<'a>)
    : Task<Result<'acc, 'e>> =
    task {
...
let inline traverseOption (func: 'a -> 'b option) (vals: seq<'a>) : seq<'b> option =
    ...
    iterStop tryOne vals
    if failed then None else Some(list :> seq<'b>)
...
let inline traverseOptionTask (func: 'a -> Task<'b option>) (vals: seq<'a>) : Task<seq<'b> option> =
    task {
        ...
        do! iterTaskStop tryOne vals
        if failed then return None else return Some(list :> seq<'b>)
    }
...
let hash (s: seq<'a>) : int =
    Seq.fold (fun a b -> HashCode.Combine(a, b)) 0 s
...
let partition (f: 'a -> bool) (vals: seq<'a>) : seq<'a> * seq<'a> =
    ...
    for v in vals do
        if f v then trues.Add(v) else falses.Add(v)
