...
let mapTask (f: 'k -> 'a -> Task<'b>) (m: Map<'k, 'a>) : Task<Map<'k, 'b>> =
    m
    |> Seq.mapTask (fun (KeyValue(k, v)) -> Task.map (fun nv -> (k, nv)) (f k v))
    |> Task.map Map.ofSeq

let mapMaybe (f: 'k -> 'a -> 'b option) (m: Map<'k, 'a>) : Map<'k, 'b> =
    m
    |> Seq.mapMaybe (fun (KeyValue(k, v)) -> Option.map (fun v' -> (k, v')) (f k v))
    |> Map.ofSeq
...
let intersectWithMaybe (resolve: 'k -> 'v -> 'v -> 'v1 option) (a: Map<'k, 'v>) (b: Map<'k, 'v>) : Map<'k, 'v1> =
    Enumerable.Join(
        ...
        (fun (KeyValue(k, v)) -> k),
        (fun (KeyValue(k, v)) -> k),
        fun (KeyValue(k1, v1)) (KeyValue(k2, v2)) -> Option.map (fun r -> (k1, r)) (resolve k1 v1 v2)
    )
    |> Seq.catMaybes
    |> Map.ofSeq
...
let update (func: 'k -> 'a -> 'b option) (map: Map<'k, 'a>) : Map<'k, 'b> =
    map
    |> Seq.mapMaybe (fun (KeyValue(k, a)) -> Option.map (fun b -> (k, b)) <| func k a)
    |> Map.ofSeq

let keysSet (map: Map<'k, 'v>) : Set<'k> = map |> Map.keys |> Set.ofSeq
...
let mapKeys (func: 'k1 -> 'k2) : Map<'k1, 'a> -> Map<'k2, 'a> =
    mapWithKeys (fun name v -> (func name, v))
