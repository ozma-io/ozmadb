module OzmaDB.OzmaUtils.Serialization.Yaml
...
type SimpleTypeConverter<'a>() =
    ...
    override this.ReadYaml (parser: IParser) (someType: Type) : obj = this.ReadTypedYaml parser :> obj
    ...
    let (readYaml, writeYaml) =
        ...
        | None ->
            ...
            | None ->
                ...
                | Some enumInfo ->
                    let optionsMap =
                        enumInfo.Cases |> Map.mapWithKeys (fun tag case -> (case.Name, case.Value))
                    ...
                    (read, write)
                | None -> failwith "(De)serialization for arbitrary unions is not implemented"
...
type UnionTypeConverter() =
    ...
    override this.Accepts(someType: Type) : bool = FSharpType.IsUnion someType
...
type SpecializedRecordConverter<'a>(converter: CrutchTypeConverter) =
    ...
    let defaultValuesMap =
        fields
        |> Seq.mapMaybe (fun (name, field) -> Option.map (fun def -> (name, def)) field.DefaultValue)
        |> Map.ofSeq
...
type RecordTypeConverter() =
    ...
    override this.Accepts(someType: Type) : bool = FSharpType.IsRecord someType
...
type MapTypeConverter() =
    ...
    override this.GetConverterType(someType: Type) : Type =
        typedefof<SpecializedMapConverter<_, _>>
            .MakeGenericType(someType.GetGenericArguments())

    override this.Accepts(someType: Type) : bool =
        someType.IsGenericType
        && someType.GetGenericTypeDefinition() = typedefof<Map<_, _>>

type SpecializedSetConverter<'k when 'k: comparison>(converter: CrutchTypeConverter) =
    interface IYamlTypeConverter with
        override this.Accepts someType = someType = typeof<Set<'k>>
...
type SetTypeConverter() =
    ...
    override this.GetConverterType(someType: Type) : Type =
        typedefof<SpecializedSetConverter<_>>
            .MakeGenericType(someType.GetGenericArguments())

    override this.Accepts(someType: Type) : bool =
        someType.IsGenericType
        && someType.GetGenericTypeDefinition() = typedefof<Set<_>>
...
type JValueConverter() =
    ...
    override this.WriteTypedYaml (emitter: IEmitter) (value: JValue) = this.Serialize(emitter, value.Value)
...
let makeYamlSerializer (settings: YamlSerializerSettings) : ISerializer =
    ...
    let converters =
        Seq.append myConverters (settings.CrutchTypeConverters()) |> Seq.cache
