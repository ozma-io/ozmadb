...
module HashSet =
    ...
    let add (item: 'a) (map: HashSet<'a>) : HashSet<'a> = map.Add(item)

    let delete (item: 'a) (map: HashSet<'a>) : HashSet<'a> = map.Remove(item)

    let contains (key: 'a) (map: HashSet<'a>) : bool = map.Contains key
