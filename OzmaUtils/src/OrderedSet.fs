...
type OrderedSet<'a> when 'a: comparison =
    ...
    member this.GetEnumerator() =
        (this.Order :> IEnumerable<'a>).GetEnumerator()

    member this.CopyTo(arr: 'a[], arrayIndex: int) = this.Order.CopyTo(arr, arrayIndex)
...
module OrderedSet =
    ...
    let ofSeqUnique (objs: 'a seq) =
        ofSeqGeneric (fun item -> failwithf "Item '%O' already exists" item) objs
    ...
    let remove (item: 'a) (map: OrderedSet<'a>) : OrderedSet<'a> = filter (fun citem -> citem = item) map

    let contains (key: 'a) (map: OrderedSet<'a>) : bool = Set.contains key map.Keys
