...
module ObjectMap =
    ...
    let ofSeq (objs: obj seq) : ObjectMap =
        objs
        |> Seq.map (fun o -> KeyValuePair(o.GetType(), o))
    ...
    let filter (f: Type -> obj -> bool) (ObjectMap map) : ObjectMap =
        map
        |> Seq.filter (fun (KeyValue(otyp, o)) -> f otyp o)
        |> ImmutableDictionary.CreateRange
        |> ObjectMap
    ...
    let remove (typ: Type) (ObjectMap map) : ObjectMap = HashMap.remove typ map |> ObjectMap
    ...
    let findType<'a> (map: ObjectMap) : 'a = find typeof<'a> map :?> 'a

    let contains (typ: Type) (ObjectMap map) : bool = map.ContainsKey typ

    let containsType<'a> (map: ObjectMap) : bool = contains typeof<'a> map
