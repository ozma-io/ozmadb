...
type JsonMap =
    ...
    interface IEquatable<JsonMap> with
        member this.Equals(other: JsonMap) = JObject.DeepEquals(this.Map, other.Map)
...
module JsonMap =
    ...
    let singleton (key: string) (item: JToken) : JsonMap = ofSeq (seq { yield key, item })

    let toSeq (map: JsonMap) : (string * JToken) seq =
        map.Map.Properties() |> Seq.map (fun p -> (p.Name, p.Value))

    let contains (key: string) (map: JsonMap) : bool = map.Map.ContainsKey key
