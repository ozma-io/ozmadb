module OzmaDB.Permissions.Entity
    ...
    let applyToField name (field: SerializedColumnField) : SerializedColumnField option =
        ...
        if not (checkRoleFields (fun _ -> true)) then
            ...
            let access: SerializedFieldAccess =
                let select =
                    entityAccess.Select
                    && checkRoleFields (fun field -> not (optimizedIsFalse field.Select))
            ...
            let field = { field with Access = access }
            Some field
