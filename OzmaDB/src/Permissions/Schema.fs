...
let private makeSourceAllowedEntity (entity : RoleEntity) : SourceAllowedEntity =
    let fields = entity.ColumnFields |> Seq.map (fun col -> (OzmaQLName col.ColumnName, makeSourceAllowedField col)) |> Map.ofSeqUnique
...
let private makeSourceRole (role : Role) : SourceRole =
    { Parents = role.Parents |> Seq.map (fun role -> { Schema = OzmaQLName role.Parent.Schema.Name; Name = OzmaQLName role.Parent.Name }) |> Set.ofSeqUnique
...
let private makeSourcePermissionsSchema (schema : Schema) : SourcePermissionsSchema =
    { Roles = schema.Roles |> Seq.map (fun role -> (OzmaQLName role.Name, makeSourceRole role)) |> Map.ofSeqUnique
    }
...
let buildSchemaPermissions (db : SystemContext) (filter : Expression<Func<Schema, bool>> option) (cancellationToken : CancellationToken) : Task<SourcePermissions> =
    task {
        ...
        let sourceSchemas = schemas |> Seq.map (fun schema -> (OzmaQLName schema.Name, makeSourcePermissionsSchema schema)) |> Map.ofSeqUnique
