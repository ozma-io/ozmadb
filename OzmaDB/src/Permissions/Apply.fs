module OzmaDB.Permissions.Apply
...
let private unionOptionalExprs
    (ma: ResolvedOptimizedFieldExpr option)
    (mb: ResolvedOptimizedFieldExpr option)
    : ResolvedOptimizedFieldExpr option =
    match (ma, mb) with
    | (Some a, Some b) -> Some(orFieldExpr a b)
    | (None, None) -> None
    | _ -> failwith "Impossible"

let private unionHalfAppliedAllowedEntities
    (a: HalfAppliedAllowedEntity)
    (b: HalfAppliedAllowedEntity)
    : HalfAppliedAllowedEntity =
...
let inline private getAccessFilter
    ...
    let allowedEntity =
        Map.findWithDefault entityRef defaultAllowedEntity flatAllowedEntity.Children
...
type private EntityAccessFilterBuilder
    (layout: Layout, flatAllowedEntity: FlatAllowedRoleEntity, flatUsedEntity: FlatUsedEntity) =
    let addSelectFilter
        (usedEntity: UsedEntity)
        (entityRef: ResolvedEntityRef)
        (allowedEntity: FlatAllowedDerivedEntity)
        =
    ...
    let addStandaloneSelectFilter (entityRef: ResolvedEntityRef) (allowedEntity: FlatAllowedDerivedEntity) =
        let usedEntity =
            Map.findWithDefault entityRef emptyUsedEntity flatUsedEntity.Children
    ...
    let rec getSelectAccessFilter =
        memoizeN (
            getAccessFilter (fun ent -> getSelectAccessFilter ent) addStandaloneSelectFilter layout flatAllowedEntity
        )
    // fsharplint:disable-next-line ReimplementsFunction
    let rec getUpdateAccessFilter =
        memoizeN (
            getAccessFilter
                (fun ent -> getUpdateAccessFilter ent)
                (fun entityRef allowedEntity -> allowedEntity.Update)
                layout
                flatAllowedEntity
        )
    // fsharplint:disable-next-line ReimplementsFunction
    let rec getDeleteAccessFilter =
        memoizeN (
            getAccessFilter
                (fun ent -> getDeleteAccessFilter ent)
                (fun entityRef allowedEntity -> allowedEntity.Delete)
                layout
                flatAllowedEntity
        )
    // fsharplint:disable-next-line ReimplementsFunction
    let rec getCheckAccessFilter =
        memoizeN (
            getAccessFilter (fun ent -> getCheckAccessFilter ent) addStandaloneCheckFilter layout flatAllowedEntity
        )
    ...
    let rec buildFilter (entityRef: ResolvedEntityRef) : EntityAccessFilter =
        ...
        let allowedEntity =
            Map.findWithDefault entityRef defaultParentAllowedEntity flatAllowedEntity.Children
        ...
        let checkExpr =
            if usedEntity.Insert || usedEntity.Update then
                ...
                | OFEFalse ->
                    raisef PermissionsApplyException "Access denied for insert/update check for entity %O" entityRef
                | checkExpr -> checkExpr
            else
                OFETrue
...
type private EntityChecksCache(layout: Layout, rootRef: ResolvedEntityRef) =
    let subEntityFieldRef =
        lazy
            (let fieldRef =
                { Entity = Some <| relaxEntityRef rootRef
                  Name = funSubEntity }
                : FieldRef
...
type private EntityFiltersCombiner
    (layout: Layout, checksCache: EntityChecksCache, getFilter: ResolvedEntityRef -> ResolvedOptimizedFieldExpr) =
    ...
    let rec buildChildrenFilter (entityRef: ResolvedEntityRef) : SubEntityCheck =
        ...
        let getChild (childRef, child) =
            if not child.Direct then
                ...
                let childrenCheck =
                    { subchildrenCheck with
                        Check = andFieldExpr filter subchildrenCheck.Check }
                    : SubEntityCheck

                Some(childRef, childrenCheck)

        let children = entity.Children |> Map.toSeq |> Seq.mapMaybe getChild |> Seq.cache
        ...
        let singleExpr =
            ...
            | Some((firstChildRef,
                    { AllowChildren = true
                      Check = firstExpr }),
                   tailChildren) when
                Seq.forall (fun (childRef, check) -> check.AllowChildren) tailChildren
        ...
        | None ->
            let childExpr =
                children
                |> Seq.map (uncurry (addTypecheck checksCache))
    ...
    let getSelfEntityCheck (entityRef: ResolvedEntityRef) = getParentFilter entityRef
...
let private renameAllFieldExprEntities (toEntityRef: EntityRef) : ResolvedFieldExpr -> ResolvedFieldExpr =
    let mapReference: LinkedBoundFieldRef -> LinkedBoundFieldRef =
        function
        | { Ref = { Ref = VRColumn { Entity = Some entityRef
                                     Name = fieldName } } } as ref ->
            { ref with
                Ref =
                    { ref.Ref with
                        Ref =
                            VRColumn
                                { Entity = Some toEntityRef
                                  Name = fieldName } } }
    ...
    mapFieldExpr
        { idFieldExprMapper with
            FieldReference = mapReference }

let private buildFinalRestriction
    (name: string)
    (entityRef: ResolvedEntityRef)
    : ResolvedOptimizedFieldExpr -> EntityFilterExpr =
    function
    | OFEFalse -> raisef PermissionsApplyException "Access denied to %s for entity %O" name entityRef
    | OFETrue -> FUnfiltered
    | expr ->
        expr.ToFieldExpr()
        |> renameAllFieldExprEntities (relaxEntityRef entityRef)
        |> FFiltered
...
let private buildFinalAllowedEntity
    (entityRef: ResolvedEntityRef)
    (allowedEntity: HalfAppliedAllowedEntity)
    : AppliedAllowedEntity =
...
let private applyPermissionsForEntity
    (layout: Layout)
    (checksCache: EntityChecksCache)
    (allowedEntity: FlatAllowedRoleEntity)
    (usedEntity: FlatUsedEntity)
    : HalfAppliedAllowedDatabase =
    let treeBuilder = EntityAccessFilterBuilder(layout, allowedEntity, usedEntity)

    let filterBuilder =
        EntityFiltersCombiner(layout, checksCache, (fun ref -> treeBuilder.GetFilter(ref).Filter))

    let checkBuilder =
        EntityFiltersCombiner(layout, checksCache, (fun ref -> treeBuilder.GetFilter(ref).Check))

    usedEntity.EntryPoints
    |> Seq.map (fun ref -> (ref, getAppliedEntity treeBuilder filterBuilder checkBuilder ref))
    |> Map.ofSeq

let inline private throwRoleExceptions
    (rootRef: ResolvedRoleRef)
    (exceptions: (ResolvedRoleRef * PermissionsApplyException) list)
    =
    let postfix =
        exceptions
        |> Seq.map (fun (roleRef, e) -> sprintf "for role %O: %O" roleRef e)
        |> String.concat ", "

    let msg = sprintf "Access denied to the root entity %O: %s" rootRef postfix

    raise
    <| PermissionsApplyException(msg, List.map (fun (roleRef, e) -> upcast e) exceptions, true)
...
let applyPermissions (layout: Layout) (role: ResolvedRole) (usedDatabase: FlatUsedDatabase) : AppliedAllowedDatabase =
    let applyToOne (rootRef, usedEntity) =
        ...
        let tryRole maybeAppliedDb (roleRef, allowedRoleEntity) =
            try
                let newAppliedDb =
                    applyPermissionsForEntity layout checksCache allowedRoleEntity usedEntity
    ...
    |> Map.toSeq
    |> Seq.map applyToOne
    |> Seq.fold Map.unionUnique Map.empty

let private checkPermissionsForEntity
    (layout: Layout)
    (allowedEntity: FlatAllowedRoleEntity)
    (usedEntity: FlatUsedEntity)
    : unit =
...
let getSingleCheckExpression
    ...
    let tryGetRoleCheck (roleRef, allowedRoleEntity: FlatAllowedRoleEntity) =
        ...
        with :? PermissionsApplyException ->
            OFEFalse

    let allowedEntity =
        match Map.tryFind entity.Root role.Flattened.Entities with
        | None ->
            raisef PermissionsApplyException "Access denied to the root entity %O for entity %O" entity.Root entityRef
        | Some allowed -> allowed
