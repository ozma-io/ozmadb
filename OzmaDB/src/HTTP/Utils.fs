...
let requestError<'Error when 'Error :> IErrorDetails> (e : 'Error) =
    setHttpHeader "X-OzmaDB-Error" e.Error >=> setStatusCode e.HTTPResponseCode >=> json e
...
let inline commitAndReturn (api : IOzmaDBAPI) ([<InlineIfLambda>] getResult : unit -> HttpJobResponse) : Task<HttpJobResponse> =
    task {
...
let inline commitAndOk (api : IOzmaDBAPI) : Task<HttpJobResponse> = commitAndReturn api (fun () -> jobJson Map.empty)

let inline jobReplyWithCommit<'Response, 'Error when 'Error :> IErrorDetails> (api : IOzmaDBAPI) (ret : Result<'Response, 'Error>) =
    match ret with
...
type HttpJobUtils (
    ...
    let runJobWithApi (touchAccessedAt : bool) (ictx : InstanceContext) (f : IOzmaDBAPI -> Task<HttpJobResponse>) (next : HttpFunc) (ctx : HttpContext) =
    ...
    let withRateLimitCheck (prefix : string) (getLimits : IInstance -> RateLimit seq) (f : IOzmaDBAPI -> Task<HttpJobResponse>) =
    ...
    member this.PerformReadJob (f : IOzmaDBAPI -> Task<HttpJobResponse>) = withRateLimitCheck "rate_limit:read:" (fun inst -> inst.ReadRateLimitsPerUser) f
    member this.PerformWriteJob (f : IOzmaDBAPI -> Task<HttpJobResponse>) = withRateLimitCheck "rate_limit:write:" (fun inst -> inst.WriteRateLimitsPerUser) f
