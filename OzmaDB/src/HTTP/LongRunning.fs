module OzmaDB.HTTP.LongRunning
...
let private remoteJobName (remote: RemoteJobRef) =
    sprintf "job:%s:%O" remote.Settings.ClientId remote.Id

let private remoteJobDataName (remote: RemoteJobRef) =
    sprintf "job:%s:%O:data" remote.Settings.ClientId remote.Id
...
let private parseRemoteResult
    (remote: RemoteJobRef)
    (connection: Redis.IDatabase)
    (rawResult: Redis.RedisValue)
...
let private waitRemoteJob<'a>
    (remote: RemoteJobRef)
    (delay: TimeSpan)
    (cancellationToken: CancellationToken)
    : Task<RemoteJobResult<'a>> =
    task {
...
let getJobDataBytes
    ...
    task {
        ...
        match writer with
        | JOAsync writer -> do! writer memoryStream cancellationToken
        | JOSync writer -> writer memoryStream cancellationToken
...
type LongRunningJob<'a>
    ...
    let runSubmitJob (job: Task<'a * JobDataWriter>) (remote: RemoteJobRef) =
        unitTask {
            try
                ...
                let! result =
                    task {
                        ...
                        | e ->
                            logger.LogError(
                                e,
                                "An unhandled exception has occurred while executing the long-running job {jobId}.",
                                remote.Id
                            )
                ...
                let! successPublish = conn.PublishAsync(Redis.RedisChannel.op_Implicit (jobName), "finished")
                ()
            finally
                cancelSource.Cancel()
    ...
    member this.Start(runJob: CancellationToken -> Task<'a * JobDataWriter>) =
        match status with
        | JSNoJob -> status <- JSLocal(runJob cancelSource.Token)
        | _ -> raisef InvalidOperationException "The job has already been started"
    ...
    member this.ConvertToRemote(settings: RemoteJobSettings) : Task<RemoteJobRef> =
        task {
            ...
            | JSLocal job ->
                ...
                let remote = { Settings = settings; Id = jobId }
                let jobName = remoteJobName remote
