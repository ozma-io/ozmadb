...
type private ModulesUpdater(db: SystemContext) as this =
    ...
    let updateModulesDatabase (schema: SourceModulesSchema) (existingSchema: Schema) : unit =
        let oldModulesMap =
            existingSchema.Modules
            |> Seq.map (fun modul -> (modul.Path, modul))
            |> Map.ofSeq

        let updateFunc _ = updateModulesField

        let createFunc modulePath =
            Module(Path = modulePath, Schema = existingSchema)

        ignore
        <| this.UpdateDifference updateFunc createFunc schema.Modules oldModulesMap
