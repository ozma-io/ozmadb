module OzmaDB.UserViews.DryRun
...
[<NoEquality; NoComparison>]
type RowAttributeInfo = { Type: SQL.SimpleValueType }
...
type ArgumentsPrettyConverter() =
    ...
    override this.ReadJson
        ...
        : OrderedMap<ArgumentName, ArgumentInfo> =
        raise <| NotImplementedException()

    override this.WriteJson
        (writer: JsonWriter, value: OrderedMap<ArgumentName, ArgumentInfo>, serializer: JsonSerializer)
        : unit =
...
let emptyPrefetchedUserViews: PrefetchedUserViews = { Schemas = Map.empty }
...
let mergePrefetchedUserViews (a: PrefetchedUserViews) (b: PrefetchedUserViews) =
    let mergeOne name a b =
        ...
        | (Ok schema1, Ok schema2) ->
            ...
            with Failure e ->
                failwithf "Error in schema %O: %s" name e
...
let private getColumn: (ColumnType * SQL.ColumnName) -> OzmaQLName option =
    function
    | (CTColumn c, _) -> Some c
    | _ -> None
...
let private getConstAttributes (info: UserViewInfo) (res: ExecutingViewExpr) : ConstAttributes =
    let pureAttrs =
        res.Attributes |> Map.filter (fun name _ -> info.AttributeTypes.[name].Pure)
...
let private addColumnTypes (info: UserViewInfo) (compiled: CompiledViewExpr) : CompiledViewExpr =
    let getColumn name =
        info.Columns |> Array.find (fun c -> c.Name = name)
    ...
    let addType (column: CompiledColumnInfo) =
        let info =
            ...
            | CTColumn name ->
                ...
                { column.Info with
                    ValueType = Some newColumn.ValueType }
    ...
    { compiled with
        Columns = Array.map addType compiled.Columns }

type private DryRunner
    (
        layout: Layout,
        triggers: MergedTriggers,
        conn: QueryConnection,
        forceAllowBroken: bool,
        onlyWithAllowBroken: bool option,
        cancellationToken: CancellationToken
    ) =
    let mutable serializedFields: Map<ResolvedFieldRef, SerializedColumnField> =
        Map.empty
    ...
    let getSerializedField (ref: ResolvedFieldRef) =
        ...
        | None ->
            ...
            | Some { Field = RColumnField src } ->
                let triggersEntity =
                    Option.defaultValue emptyMergedTriggersEntity (triggers.FindEntity ref.Entity)
    ...
    let mergeViewInfo
        ...
        (viewInfo: ExecutedViewInfo)
        (results: ExecutingViewExpr)
        : UserViewInfo =
        ...
        let constAttrsNames =
            compiled.SingleRowQuery.ConstColumns |> Seq.mapMaybe getConstAttr |> Set.ofSeq
        ...
        let constColumnAttrNames =
            compiled.SingleRowQuery.ConstColumns
            |> Seq.mapMaybe getConstColunmAttr
            |> Set.ofSeq
        ...
        let constArgumentAttrNames =
            compiled.SingleRowQuery.ConstColumns
            |> Seq.mapMaybe getConstArgumentAttr
            |> Set.ofSeq
        ...
        let getColumnAttributeMapping (column: CompiledColumnInfo) =
            match column.Type with
            | CTColumnMeta(colName, CCCellAttribute attrName) ->
                Option.map (Map.singleton attrName >> Map.singleton colName) column.Info.Mapping
            | _ -> None

        let columnAttributeMappings =
            Seq.mapMaybe getColumnAttributeMapping allSingleRowColumnsInfo
            |> Seq.fold (Map.unionWith Map.unionUnique) Map.empty

        let cellAttributeMappings =
            Seq.mapMaybe getColumnAttributeMapping compiled.Columns
            |> Seq.fold (Map.unionWith Map.unionUnique) Map.empty

        let getArgumentAttributeMapping (column: CompiledColumnInfo) =
            match column.Type with
            | CTMeta(CMArgAttribute(argName, attrName)) ->
                Option.map (Map.singleton attrName >> Map.singleton argName) column.Info.Mapping
            | _ -> None

        let argumentAttributeMappings =
            Seq.mapMaybe getArgumentAttributeMapping allSingleRowColumnsInfo
            |> Seq.fold (Map.unionWith Map.unionUnique) Map.empty
        ...
        let getResultColumn (column: ExecutedColumnInfo) : UserViewColumn =
            ...
            let columnMappings =
                Map.findWithDefault column.Name Map.empty columnAttributeMappings
        ...
        let attributesStr =
            ...
            |> Seq.map (fun (info, expr) -> SQL.SCExpr(Some info.Name, expr) |> string)
            |> String.concat ", "

        let queryStr = compiled.Query.Expression.ToString()

        let hash =
            String.concatWithWhitespaces [ attributesStr; queryStr ]
            |> Hash.sha1OfString
            |> String.hexBytes
        ...
        let getArg arg (info: ResolvedArgument) =
            ...
            | PLocal name ->
                let argAttrTypes =
                    Map.tryFind name viewInfo.ArgumentAttributeTypes
                ...
                let getAttributeInfo attrName typ =
                    ...
                    let mapping = if isConst then None else Map.tryFind attrName argMappings
        ...
        let getRowAttributeInfo attrName typ = { Type = typ }
    ...
    let rec dryRunUserView (uv: ResolvedUserView) (comment: string option) : Task<PrefetchedUserView> =
        task {
            ...
            let arguments =
                uv.Compiled.Query.Arguments.Types
                |> Map.map (fun name arg -> defaultCompiledArgument arg)

            try
                return!
                    runViewExpr conn layout limited comment arguments cancellationToken
                    <| fun info res ->
                        ...
                        let nonpureCompiled =
                            { uv.Compiled with
                                SingleRowQuery =
                                    { uv.Compiled.SingleRowQuery with
                                        ConstColumns = [||] } }

                        let nonpureUv =
                            { uv with
                                Compiled = addColumnTypes mergedInfo nonpureCompiled }

                        Task.FromResult
                            { UserView = nonpureUv
                              Info = mergedInfo
                              ConstAttributes = getConstAttributes mergedInfo res }
            with :? UserViewExecutionException as err ->
    ...
    let resolveUserViewsSchema (schemaName: SchemaName) (schema: UserViewsSchema) : Task<PrefetchedViewsSchema> =
        task {
                ...
                task {
                    try
                        ...
                        | Ok uv ->
                            ...
                            with :? UserViewDryRunException as e when uv.AllowBroken || forceAllowBroken ->
                                return
                                    Some(
                                        name,
                                        Error
            ...
            let! userViews =
                schema.UserViews
                |> Map.toSeq
                |> Seq.mapTask mapUserView
                |> Task.map (Seq.catMaybes >> Map.ofSeq)
...
let dryRunUserViews
    ...
    (userViews: UserViews)
    (cancellationToken: CancellationToken)
    : Task<PrefetchedUserViews> =
    task {
        let runner =
            DryRunner(layout, triggers, conn, forceAllowBroken, onlyWithAllowBroken, cancellationToken)

        return! runner.DryRunUserViews userViews
    }

let dryRunAnonymousUserView
    (conn: QueryConnection)
    (layout: Layout)
    (triggers: MergedTriggers)
    (q: ResolvedUserView)
    (cancellationToken: CancellationToken)
    : Task<PrefetchedUserView> =
