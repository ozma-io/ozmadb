module OzmaDB.UserViews.Update
...
type private UserViewsUpdater(db: SystemContext) as this =
    ...
    let updateUserViewsSchema (schema: SourceUserViewsSchema) (existingSchema: Schema) : unit =
        ...
        let oldUserViewsMap =
            existingSchema.UserViews
            |> Seq.map (fun uv -> (OzmaQLName uv.Name, uv))
            |> Map.ofSeq

        let updateFunc _ = updateUserView

        let createFunc (OzmaQLName name) =
            let newUv = UserView(Name = name)
            existingSchema.UserViews.Add(newUv)
            newUv

        ignore
        <| this.UpdateDifference updateFunc createFunc schema.UserViews oldUserViewsMap
...
type private UserViewErrorRef =
    | ERGenerator of SchemaName
    ...
    task {
        ...
        let genChecks =
            ...
            |> Seq.mapMaybe (function
                | ERGenerator ref -> Some ref
                | _ -> None)
            |> Seq.map checkUserViewGeneratorSchema
        ...
        let uvChecks =
            broken
            |> Seq.mapMaybe (function
                | ERUserView ref -> Some ref
                | _ -> None)
            |> Seq.map checkUserViewName
