...
type private AttributesUpdater(db: SystemContext, allSchemas: Schema seq) as this =
    ...
    let updateAttributesDatabase (schema: SourceAttributesDatabase) (existingSchema: Schema) : unit =
        let addOldAttrsKey (attrs: FieldAttributes) =
            ({ Entity =
                { Schema = OzmaQLName attrs.FieldEntity.Schema.Name
                  Name = OzmaQLName attrs.FieldEntity.Name }
               Name = OzmaQLName attrs.FieldName },
             attrs)
        ...
        let addNewAttrsEntityKey schemaName (entityName, entity: SourceAttributesEntity) =
            entity.Fields
            |> Map.toSeq
            |> Seq.map (addNewAttrsFieldKey schemaName entityName)
        ...
        let newAttrsMap =
            schema.Schemas |> Map.toSeq |> Seq.collect addNewAttrsSchemaKey |> Map.ofSeq
        ...
        let createFunc (fieldRef: ResolvedFieldRef) =
            ...
            FieldAttributes(FieldEntity = entity, FieldName = string fieldRef.Name, Schema = existingSchema)
...
let private findBrokenAttributesEntity
    (schemaName: SchemaName)
    (attrEntityRef: ResolvedEntityRef)
    (entity: AttributesEntity)
    : DefaultAttributeRef seq =
    seq {
        for KeyValue(attrFieldName, maybeField) in entity.Fields do
            ...
            | Error e when not e.AllowBroken ->
                yield
                    { Schema = schemaName
                      Field =
                        { Entity = attrEntityRef
                          Name = attrFieldName } }
            | _ -> ()
    }

let private findBrokenAttributesSchema
    (schemaName: SchemaName)
    (attrSchemaName: SchemaName)
    (schema: AttributesSchema)
    : DefaultAttributeRef seq =
    seq {
        for KeyValue(attrEntityName, entity) in schema.Entities do
            yield!
                findBrokenAttributesEntity
                    schemaName
...
let private checkAttributeName (ref: DefaultAttributeRef) : Expr<FieldAttributes -> bool> =
    ...
    <@
        fun attrs ->
            (%checkSchema) attrs.Schema
            && (%checkEntity) attrs.FieldEntity
