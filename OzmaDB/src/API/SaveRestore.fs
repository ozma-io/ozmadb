...
let private canSave: RoleType -> bool =
    function
    | RTRoot -> true
    | RTRole role -> role.CanRead

let private canRestore: RoleType -> bool =
    function
    | RTRoot -> true
    | RTRole _ -> false
...
let trySchemasFromZipFile (stream: Stream) =
    ...
    with :? RestoreSchemaException as e when e.IsUserException ->
        Error(RRERequest <| fullUserMessage e)

type SaveRestoreAPI(api: IOzmaDBAPI) =
    ...
    member this.SaveSchemas(req: SaveSchemasRequest) : Task<Result<SaveSchemasResponse, SaveErrorInfo>> =
        wrapAPIResult rctx "saveSchemas" req
        <| fun () ->
            task {
                let schemas =
                    match req with
                    | SRSpecified schemas -> Map.toSeq schemas
                    | SRAll settings -> ctx.Layout.Schemas |> Map.keys |> Seq.map (fun name -> (name, settings))
                            ...
                            if not <| Map.containsKey name ctx.Layout.Schemas then
                                ...
                                let! schema =
                                    if onlyCustomEnttieies then
                                        task {
                                            let! customEntities =
                                                saveCustomEntities ctx.Transaction ctx.Layout name ctx.CancellationToken
                                        ...
                                        task {
                                            let! schemaDump =
                                                saveSchema ctx.Transaction ctx.Layout name ctx.CancellationToken
                    ...
                    if not <| Set.isEmpty (Set.intersect fullRestoredSchemas preloadedSchemas) then
                        ...
                        try
                            ...
                            let! modifiedMetadata =
                                restoreSchemas ctx.Transaction ctx.Layout fullSchemas ctx.CancellationToken
