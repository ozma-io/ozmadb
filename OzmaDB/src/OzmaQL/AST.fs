...
and [<TypeConverter(typeof<NewtypeConverter<OzmaQLName>>)>] [<Struct>] OzmaQLName = OzmaQLName of string
    with
...
type ResolvedEntityRef =
    { Schema : SchemaName
      Name : EntityName
        ...
        member this.ToOzmaQLString () = sprintf "%s.%s" (this.Schema.ToOzmaQLString()) (this.Name.ToOzmaQLString())
...
type ResolvedFieldRef =
    { Entity : ResolvedEntityRef
      Name : FieldName
        ...
        member this.ToOzmaQLString () = sprintf "%s.%s" (this.Entity.ToOzmaQLString()) (this.Name.ToOzmaQLString())
...
type [<StructuralEquality; NoComparison; SerializeAsObject("type")>] ScalarFieldType<'e> when 'e :> IOzmaQLName =
    | [<CaseKey("int")>] SFTInt
...
type [<NoEquality; NoComparison>] FromEntity<'e> when 'e :> IOzmaQLName =
...
type [<NoEquality; NoComparison>] BoundMapping =
    { [<JsonConverter(typeof<BoundMappingEntriesPrettyConverter>)>]
      ...
      Default : FieldValue option
        ...
        member this.ToOzmaQLString () =
            let esStr = this.Entries |> HashMap.toSeq |> Seq.map (fun (value, e) -> sprintf "WHEN %s THEN %s" (toOzmaQLString value) (toOzmaQLString e)) |> String.concat " "
            let elsStr =
...
and FieldExpr<'e, 'f> when 'e :> IOzmaQLName and 'f :> IOzmaQLName =
    ...
    with
        ...
        member this.ToOzmaQLString () =
            ...
            | FEAny (e, op, arr) -> sprintf "(%s) %s ANY (%s)" (e.ToOzmaQLString()) (op.ToOzmaQLString()) (arr.ToOzmaQLString())
            | FEAll (e, op, arr) -> sprintf "(%s) %s ALL (%s)" (e.ToOzmaQLString()) (op.ToOzmaQLString()) (arr.ToOzmaQLString())
            ...
            | FECase (es, els) ->
                let esStr = es |> Seq.map (fun (cond, e) -> sprintf "WHEN %s THEN %s" (cond.ToOzmaQLString()) (e.ToOzmaQLString())) |> String.concat " "
                let elsStr =
                    match els with
                    | None -> ""
                    | Some e -> sprintf "ELSE %s" (e.ToOzmaQLString())
            ...
            | FEMatch (expr, es, els) ->
                let esStr = es |> Seq.map (fun (value, e) -> sprintf "WHEN %s THEN %s" (toOzmaQLString value) (toOzmaQLString e)) |> String.concat " "
                let elsStr =
                    match els with
                    | None -> ""
                    | Some e -> sprintf "ELSE %s" (e.ToOzmaQLString())
            ...
            | FEJsonObject obj -> obj |> Map.toSeq |> Seq.map (fun (k, v) -> sprintf "%s: %s" (k.ToOzmaQLString()) (v.ToOzmaQLString())) |> String.concat ", " |> sprintf "{%s}"
            | FEFunc (name, args) -> sprintf "%s(%s)" (name.ToOzmaQLString()) (args |> Seq.map toOzmaQLString |> String.concat ", ")
      ...
      Offset : FieldExpr<'e, 'f> option
        ...
        member this.ToOzmaQLString () =
            let orderByStr =
                ...
                then ""
                else sprintf "ORDER BY %s" (this.OrderBy |> Seq.map toOzmaQLString |> String.concat ", ")
            let limitStr =
...
and [<NoEquality; NoComparison>] SingleSelectExpr<'e, 'f> when 'e :> IOzmaQLName and 'f :> IOzmaQLName =
    { Attributes : AttributesMap<'e, 'f>
      ...
      Extra : ObjectMap
        ...
        member this.ToOzmaQLString () =
            let attributesStrs = this.Attributes |> Map.toSeq |> Seq.map (fun (name, expr) -> sprintf "@%s = %s" (name.ToOzmaQLString()) (expr.ToOzmaQLString()))
            ...
            sprintf "SELECT %s" (String.concatWithWhitespaces [resultStr; fromStr; whereStr; groupByStr; this.OrderLimit.ToOzmaQLString()])
    ...
    { Operation : SetOperation
      ...
      OrderLimit : OrderLimitClause<'e, 'f>
        ...
        member this.ToOzmaQLString () =
            ...
            String.concatWithWhitespaces [aStr; this.Operation.ToOzmaQLString(); allowDuplicatesStr; bStr; this.OrderLimit.ToOzmaQLString()]
...
and [<NoEquality; NoComparison>] JoinExpr<'e, 'f> when 'e :> IOzmaQLName and 'f :> IOzmaQLName =
    ...
    with
        ...
        member this.ToOzmaQLString () =
            sprintf "(%s %s JOIN %s ON %s)" (this.A.ToOzmaQLString()) (this.Type.ToOzmaQLString()) (this.B.ToOzmaQLString()) (this.Condition.ToOzmaQLString())
...
type FieldExprMapper<'e1, 'f1, 'e2, 'f2> when 'e1 :> IOzmaQLName and 'f1 :> IOzmaQLName and 'e2 :> IOzmaQLName and 'f2 :> IOzmaQLName =
...
type FieldExprTaskMapper<'e1, 'f1, 'e2, 'f2> when 'e1 :> IOzmaQLName and 'f1 :> IOzmaQLName and 'e2 :> IOzmaQLName and 'f2 :> IOzmaQLName =
...
type OzmaQLVoid = private OzmaQLVoid of unit with
    interface IOzmaQLString with
...
let globalArgumentTypes : Map<ArgumentName, ResolvedArgument> =
    Map.ofSeq
        [ (OzmaQLName "lang", requiredArgument (FTScalar SFTString))
          ...
          (OzmaQLName "user_id", requiredArgument (FTScalar (SFTReference ({ Schema = funSchema; Name = funUsers }, None))))
