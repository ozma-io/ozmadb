...
let renderOzmaQLDateTime (j: Instant) : string =
    sprintf "%s :: datetime" (j |> renderSqlDateTime |> renderOzmaQLString)

let renderOzmaQLDate (j: LocalDate) : string =
    sprintf "%s :: date" (j |> renderSqlDate |> renderOzmaQLString)

let renderOzmaQLInterval (ts: Period) : string =
    sprintf "%s :: interval" (ts |> renderSqlInterval |> renderOzmaQLString)

let renderOzmaQLUuid (uuid: Guid) : string =
    sprintf "%s :: uuid" (uuid |> string |> renderOzmaQLString)
...
let rec renderOzmaQLJson (j: JToken) : string =
    ...
    | JTokenType.Object ->
        ...
        obj
        |> Seq.map (fun (KeyValue(k, v)) -> sprintf "%s: %s" (escapeSqlDoubleQuotes k) (renderOzmaQLJson v))
        |> String.concat ","
        |> sprintf "{ %s }"
    | JTokenType.Array ->
...
let renderAttributesMap<'e, 'f when 'e :> IOzmaQLString and 'e: comparison and 'f :> IOzmaQLString>
    (attrs: Map<'e, 'f>)
    =
    attrs
    |> Map.toSeq
    |> Seq.map (fun (name, e) -> sprintf "%s = %s" (name.ToOzmaQLString()) (e.ToOzmaQLString()))
    |> String.concat ", "
    |> sprintf "@{ %s }"

let toOzmaQLString<'a when 'a :> IOzmaQLString> (o: 'a) = o.ToOzmaQLString()

let optionToOzmaQLString<'a when 'a :> IOzmaQLString> : 'a option -> string =
    function
    | None -> ""
