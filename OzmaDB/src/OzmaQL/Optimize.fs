...
type HashedFieldExprs<'e, 'f> when 'e :> IOzmaQLName and 'f :> IOzmaQLName = Map<ExpressionKey, OptimizedFieldExpr<'e, 'f>>
...
and [<CustomEquality; NoComparison>] OptimizedFieldExpr<'e, 'f> when 'e :> IOzmaQLName and 'f :> IOzmaQLName =
    ...
    with
        ...
        member this.ToOzmaQLString () =
            match this with
            | OFEOr vals -> vals |> Map.values |> Seq.map (fun v -> sprintf "(%s)" (v.ToOzmaQLString())) |> String.concat " OR "
            | OFEAnd vals -> vals |> Map.values |> Seq.map (fun v -> sprintf "(%s)" (v.ToOzmaQLString())) |> String.concat " AND "
