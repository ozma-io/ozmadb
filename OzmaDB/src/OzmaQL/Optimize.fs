...
type HashedFieldExprs<'e, 'f> when 'e :> IOzmaQLName and 'f :> IOzmaQLName =
    Map<ExpressionKey, OptimizedFieldExpr<'e, 'f>>

and [<CustomEquality; NoComparison>] OptimizedFieldExpr<'e, 'f> when 'e :> IOzmaQLName and 'f :> IOzmaQLName =
    ...
    member this.ToFieldExpr() =
        ...
        | OFEOr vals ->
            vals
            |> Map.values
            |> Seq.map (fun v -> v.ToFieldExpr())
            |> Seq.fold1 (curry FEOr)
        | OFEAnd vals ->
            vals
            |> Map.values
            |> Seq.map (fun v -> v.ToFieldExpr())
            |> Seq.fold1 (curry FEAnd)
    ...
    member this.ToOzmaQLString() =
        ...
        | OFEOr vals ->
            vals
            |> Map.values
            |> Seq.map (fun v -> sprintf "(%s)" (v.ToOzmaQLString()))
            |> String.concat " OR "
        | OFEAnd vals ->
            vals
            |> Map.values
            |> Seq.map (fun v -> sprintf "(%s)" (v.ToOzmaQLString()))
            |> String.concat " AND "
        | OFENot expr -> sprintf "NOT (%s)" (expr.ToOzmaQLString())
        | OFETrue -> "TRUE"
        | OFEFalse -> "FALSE"
        | OFEExpr e -> e.String
...
and private invertHashedExprs exprs =
    exprs
    |> Map.values
    |> Seq.map (notFieldExpr >> (fun x -> (string x, x)))
    |> Map.ofSeq

let optimizeFieldValue: FieldValue -> OptimizedFieldExpr<'e, 'f> =
    function
    | FBool false -> OFEFalse
    | FBool true -> OFETrue
    | v -> FEValue v |> String.comparable |> OFEExpr

let optimizedIsFalse: OptimizedFieldExpr<'e, 'f> -> bool =
    function
    | OFEFalse -> true
    | _ -> false

let optimizedIsTrue: OptimizedFieldExpr<'e, 'f> -> bool =
    function
    | OFETrue -> true
    | _ -> false

let rec optimizeFieldExpr: FieldExpr<'e, 'f> -> OptimizedFieldExpr<'e, 'f> =
    function
...
let rec mapOptimizedFieldExpr
    (f: FieldExpr<'e1, 'f1> -> FieldExpr<'e2, 'f2>)
    (e: OptimizedFieldExpr<'e1, 'f1>)
    : OptimizedFieldExpr<'e2, 'f2> =
    match e with
    | OFEOr ors -> ors |> Map.values |> Seq.map (mapOptimizedFieldExpr f) |> Seq.fold1 orFieldExpr
    | OFEAnd ands ->
        ands
        |> Map.values
        |> Seq.map (mapOptimizedFieldExpr f)
        |> Seq.fold1 andFieldExpr
