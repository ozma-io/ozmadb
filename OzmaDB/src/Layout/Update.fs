...
type private LayoutUpdater (db : SystemContext) as this =
    ...
    let updateIndexes (entity : Entity) : Map<FieldName, SourceIndex> -> Map<FieldName, Index> -> Map<FieldName, Index> =
        ...
        let createIndexFunc (OzmaQLName name) =
            Index (
                Name = name,
                Entity = entity
            )
    ...
    let updateEntity (entity : SourceEntity) (existingEntity : Entity) : unit =
        let columnFieldsMap = existingEntity.ColumnFields |> Seq.ofObj |> Seq.map (fun col -> (OzmaQLName col.Name, col)) |> Map.ofSeq
        let computedFieldsMap = existingEntity.ComputedFields |> Seq.ofObj |> Seq.map (fun comp -> (OzmaQLName comp.Name, comp)) |> Map.ofSeq
        let uniqueConstraintsMap = existingEntity.UniqueConstraints |> Seq.ofObj |> Seq.map (fun unique -> (OzmaQLName unique.Name, unique)) |> Map.ofSeq
        let checkConstraintsMap = existingEntity.CheckConstraints |> Seq.ofObj |> Seq.map (fun check -> (OzmaQLName check.Name, check)) |> Map.ofSeq
        let indexesMap = existingEntity.Indexes |> Seq.ofObj |> Seq.map (fun index -> (OzmaQLName index.Name, index)) |> Map.ofSeq
    ...
    let updateSchema (schema : SourceSchema) (existingSchema : Schema) =
        let entitiesMap = existingSchema.Entities |> Seq.ofObj |> Seq.map (fun entity -> (OzmaQLName entity.Name, entity)) |> Map.ofSeq
