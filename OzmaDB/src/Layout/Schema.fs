...
let private makeSourceEntity (entity : Entity) : SourceEntity =
    { ColumnFields = entity.ColumnFields |> Seq.map (fun col -> (OzmaQLName col.Name, makeSourceColumnField col)) |> Map.ofSeqUnique
      ComputedFields = entity.ComputedFields |> Seq.map (fun comp -> (OzmaQLName comp.Name, makeSourceComputedField comp)) |> Map.ofSeqUnique
      UniqueConstraints = entity.UniqueConstraints |> Seq.map (fun constr -> (OzmaQLName constr.Name, makeSourceUniqueConstraint constr)) |> Map.ofSeqUnique
      CheckConstraints = entity.CheckConstraints |> Seq.map (fun constr -> (OzmaQLName constr.Name, makeSourceCheckConstraint constr)) |> Map.ofSeqUnique
      Indexes = entity.Indexes |> Seq.map (fun index -> (OzmaQLName index.Name, makeSourceIndex index)) |> Map.ofSeqUnique
      ...
      SaveRestoreKey =
        ...
        then None
        else Some <| OzmaQLName entity.SaveRestoreKey
      IsHidden = false
      IsFrozen = entity.IsFrozen
      Parent =
        if entity.Parent = null
        then None
        else Some { Schema = OzmaQLName entity.Parent.Schema.Name; Name = OzmaQLName entity.Parent.Name }
...
let private makeSourceSchema (schema : Schema) : SourceSchema =
    { Entities = schema.Entities |> Seq.map (fun entity -> (OzmaQLName entity.Name, makeSourceEntity entity)) |> Map.ofSeqUnique
