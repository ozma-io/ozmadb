...
let private checkSchemaName (OzmaQLName str as name) : unit =
    if str.StartsWith("pg_") || str = "information_schema" then
        raisef ResolveLayoutException "Invalid schema name"
    else
        checkName name

let private checkFieldName (name: OzmaQLName) : unit =
    if name = funId || name = funSubEntity then
        raisef ResolveLayoutException "Name is forbidden"
    else
        checkName name

let private reduceDefaultExpr: ParsedFieldExpr -> FieldValue option =
    function
    ...
    | FECast(FEValue value, typ) ->
        ...
        | (FStringArray vals, FTArray SFTDate) ->
            Option.map (FDateArray << Array.ofSeq) (Seq.traverseOption SQL.trySqlDate vals)
        | (FStringArray vals, FTArray SFTDateTime) ->
            Option.map (FDateTimeArray << Array.ofSeq) (Seq.traverseOption SQL.trySqlDateTime vals)
        | (FStringArray vals, FTArray SFTInterval) ->
            Option.map (FIntervalArray << Array.ofSeq) (Seq.traverseOption SQL.trySqlInterval vals)
        | _ -> None
    | _ -> None
...
let private makeHashNameFor (maxLength: int) name =
    if String.length name <= maxLength then
        ...
        ignore
        <| stringBuilder.Append(String.truncate (maxLength - hashNameSuffixLength) name)
...
let private makeHashName (OzmaQLName name) = makeHashNameFor hashNameLength name
...
type private HalfResolvedEntity =
    ...
    member private this.FindComputedField name =
        Option.map (fun x -> x :> IComputedFieldBits)
        <| Map.tryFind name this.ComputedFields
...
let private resolveUniqueConstraint
    (columnFields: Map<FieldName, ResolvedColumnField>)
    (constrName: ConstraintName)
    (constr: SourceUniqueConstraint)
    ...
    with Failure msg ->
        raisef ResolveLayoutException "Unique constraint field names clash: %s" msg
...
type private Phase1Resolver(layout: SourceLayout) =
        ...
        | (Some parentVirtual, Some childVirtual) ->
            ...
            { child with
                Virtual =
                    Some
                        { childVirtual with
                            InheritedFrom = Some newInheritedFrom }
                // Always use root column name.
                ColumnName = parent.ColumnName }
        | _ -> raisef ResolveLayoutException "Computed field names clash: %O" name
    ...
    let resolveFieldType
        ...
        try
            ...
            | FTArray SFTUserViewRef ->
                raisef ResolveLayoutException "User view references are not supported as field types"
            | FTArray(SFTReference(ref, opts)) ->
                raisef ResolveLayoutException "Arrays of references are not supported as field types"
            ...
            | FTScalar(SFTReference(refEntityRef, opts)) as t ->
                ...
                | RDANoAction -> ()
                | RDACascade -> cascadeDeletedEntities <- Set.add fieldRef.Entity cascadeDeletedEntities
                | RDASetNull ->
                ...
                referencingFields <-
                    Map.addWith Map.unionUnique refEntityRef (Map.singleton fieldRef deleteAction) referencingFields

                t
            | t -> t
        with :? QueryResolveException as e ->
            raisefWithInner ResolveLayoutException e ""

    let resolveColumnField
        (entityRef: ResolvedEntityRef)
        (entity: SourceEntity)
        (fieldName: FieldName)
        (col: SourceColumnField)
        : ResolvedColumnField =
        let fieldRef = { Entity = entityRef; Name = fieldName }

        let fieldType =
            match parse tokenizeOzmaQL fieldType col.Type with
        ...
        let defaultValue =
            ...
            | None -> None
            | Some def ->
                match parse tokenizeOzmaQL fieldExpr def with
                | Ok r ->
    ...
    let resolveEntity
        (parent: HalfResolvedEntity option)
        (entityRef: ResolvedEntityRef)
        (entity: SourceEntity)
        : HalfResolvedEntity =
        let root =
        ...
        let mapColumnField name field =
            ...
            with e ->
                raisefWithInner ResolveLayoutException e "In column field %O" name
                ...
                { Source = field
                  ...
                  Virtual =
                    if field.IsVirtual then
                        Some { InheritedFrom = None }
                    else
                        None
                  ColumnName = sqlColumnName entityRef entity name
                  HashName = makeHashName name }
            with e ->
                raisefWithInner ResolveLayoutException e "In computed field %O" name
        ...
        let columnFields =
            ...
            | Some p ->
                ...
                with Failure msg ->
                    raisef ResolveLayoutException "Column field names clash: %s" msg
            ...
            | Some p ->
                ...
                let inheritedFields =
                    Map.map
                        (fun name field ->
                            { field with
                                InheritedFrom = addParent field.InheritedFrom }
                            : HalfResolvedComputedField)
                        p.ComputedFields
        ...
        try
            ...
            let columnNames =
                selfColumnFields |> Map.values |> Seq.map (fun field -> field.HashName)

            let computedNames =
                selfComputedFields |> Map.values |> Seq.map (fun field -> field.HashName)

            Seq.append columnNames computedNames |> Set.ofSeqUnique |> ignore
        with Failure msg ->
            raisef ResolveLayoutException "Field names hash clash: %s" msg
        ...
        let mapUniqueConstraint name constr =
            ...
            with e ->
                raisefWithInner ResolveLayoutException e "In unique constraint %O" name
        ...
        let uniqueConstraints =
            ...
            | Some p ->
                ...
                let inheritedConstrs =
                    Map.map (fun name field -> addParent field) p.UniqueConstraints
        ...
        let uniqueConstraintNames =
            let makeName (name, c: ResolvedUniqueConstraint) =
                (uniqueConstraintSQLName hashName c.HashName, { Entity = entityRef; Name = name })

            try
                selfUniqueConstraints |> Map.toList |> Seq.map makeName |> Map.ofSeqUnique
            with Failure msg ->
                raisef
                    ResolveLayoutException
                    "Unique constraint names clash (first %i characters): %s"
                    hashNameLength
                    msg
        ...
        let failKey parentKey currKey =
            raisef
                ResolveLayoutException
                "Entity %O overrides save-restore key of parent entity %O, which is forbidden"
                entityRef
                (Option.get entity.Parent)

        let saveRestoreKey =
            Option.unionWith failKey parentSaveRestoreKey entity.SaveRestoreKey
        ...
        let getFlag f =
            Option.defaultValue false (Option.map f parent)
                ...
                | None ->
                    if
                        not (
                            ...
                            || Map.containsKey mainField computedFields
                        )
                    then
        ...
        | Some parentRef ->
            ...
            cachedEntities <-
                Map.add
                    parentRef
                    { oldParent with
                        Children = Map.add childRef { Direct = false } oldParent.Children }
                    cachedEntities

            addIndirectChildren childRef oldParent

    let rec resolveOneEntity
        (stack: Set<ResolvedEntityRef>)
        (child: ResolvedEntityRef option)
                ...
                | Some childRef ->
                    cachedEntities <-
                        Map.add
                            ref
                            { cached with
                                Children = Map.add childRef { Direct = true } cached.Children }
                            cachedEntities
            ...
            | None ->
                ...
                let entityWithChild =
                    ...
                    | Some childRef ->
                        ...
                        { entity with
                            Children = Map.singleton childRef { Direct = true } }

                cachedEntities <- Map.add ref entityWithChild cachedEntities
                entity
        with e ->
            raisefWithInner ResolveLayoutException e "In entity %O" ref
    ...
    let resolveLayout () =
        let iterSchema name schema =
            ...
            with e ->
                raisefWithInner ResolveLayoutException e "In schema %O" name
...
let private fillReferencingFields
    (rootEntities: ResolvedEntityRef seq)
    (entities: HalfResolvedEntitiesMap)
    (initialReferencesMap: ReferencingEntitiesMap)
    : ReferencingEntitiesMap =
...
let private fillCascadeDeletedEntities
    (entities: HalfResolvedEntitiesMap)
    (initialCascadeDeletedEntities: Set<ResolvedEntityRef>)
    : Set<ResolvedEntityRef> =
...
let private parseRelatedExpr (rawExpr: string) =
    match parse tokenizeOzmaQL fieldExpr rawExpr with
    | Ok r -> r
    | Error msg -> raisef ResolveLayoutException "Error parsing local expression: %s" msg

let private relatedResolutionFlags =
...
let rec private sortSaveRestoredEntities'
    (visited: Set<ResolvedEntityRef>)
    (saveRestoredParents: (ResolvedEntityRef * ResolvedEntityRef) seq)
    =
    if Seq.isEmpty saveRestoredParents then
        Seq.empty
    else
        let (currentGen, nextGen) =
            Seq.partition (fun (ref, parentRef) -> Set.contains parentRef visited) saveRestoredParents
...
let private sortSaveRestoredEntities (saveRestoredParents: (ResolvedEntityRef * ResolvedEntityRef option) seq) =
    let (rootGen, nextGen) =
        Seq.partition (fun (ref, maybeParentRef) -> Option.isNone maybeParentRef) saveRestoredParents
...
type private Phase2Resolver
    ...
    let mutable cachedComputedFields: Map<ResolvedFieldRef, PossiblyBroken<ResolvedComputedField>> =
        Map.empty

    let mutable cachedCaseExpressions: Map<ResolvedFieldRef, VirtualFieldCase array> =
        Map.empty

    let mutable cachedSaveRestoredEntities: Map<ResolvedEntityRef, ResolvedEntityRef option> =
        Map.empty
    ...
    let rec makeWrappedLayout (stack: Set<ResolvedFieldRef>) : ILayoutBits =
        ...
        let wrapEntity (currRef: ResolvedEntityRef) (currEnt: HalfResolvedEntity) =
            ...
            let findComputedFieldBits name =
                Option.map (fun x -> x :> IComputedFieldBits)
                <| Map.tryFind name currEnt.ComputedFields
            ...
            { new IEntityBits with
                ...
                member this.Fields =
                    ...
                    let compFields =
                        currEnt.ComputedFields
                        |> Map.toSeq
                        |> Seq.map (fun (name, comp) -> (name, RComputedField(comp :> IComputedFieldBits)))
    ...
    and resolveRelatedExpr
        (wrappedLayout: ILayoutBits)
        (entityRef: ResolvedEntityRef)
    ...
    and resolveMyVirtualCases (entity: HalfResolvedEntity) (fieldRef: ResolvedFieldRef) : VirtualFieldCase array =
        ...
        let handledChildren =
            childVirtualCases |> Seq.map (fun c -> c.PossibleEntities) |> Set.unionMany
        ...
        let childrenEntities =
            entity.Children |> Map.keys |> Seq.map (fun ref -> (ref, Map.find ref entities))

        let allEntities =
            Seq.append (Seq.singleton (fieldRef.Entity, entity)) childrenEntities
    ...
    and findVirtualChildren (entity: HalfResolvedEntity) (fieldName: FieldName) : VirtualFieldCase seq =
        entity.Children
        |> Map.toSeq
        |> Seq.collect (fun (ref, child) -> findVirtualChild fieldName ref child)

    and findVirtualChild
        (fieldName: FieldName)
        (entityRef: ResolvedEntityRef)
        (child: ChildEntity)
        : VirtualFieldCase seq =
        if not child.Direct then
            ...
            | comp when Option.isNone comp.InheritedFrom ->
                resolveVirtualCases childEntity { Entity = entityRef; Name = fieldName }
                |> Array.toSeq
            | _ -> findVirtualChildren childEntity fieldName

    and resolveOneComputeField
        (stack: Set<ResolvedFieldRef>)
        (fieldRef: ResolvedFieldRef)
        (entity: HalfResolvedEntity)
        (comp: HalfResolvedComputedField)
        ...
        let (exprInfo, expr) =
            resolveRelatedExpr wrappedLayout fieldRef.Entity (parseRelatedExpr comp.Source.Expression)
        ...
        if comp.Source.IsMaterialized then
            ...
            if usedReferences.HasRestrictedEntities then
                raisef
                    ResolveLayoutException
                    "Restricted (plain) join arrows are not allowed in materialized computed fields"

        let maybeExprType =
            try
                typecheckFieldExpr wrappedLayout expr
            with :? ViewTypecheckException as e ->
                raisefWithInner ResolveLayoutException e "Failed to typecheck computed field"

        let exprType =
            Option.getOrFailWith
                (fun () -> raisef ResolveLayoutException "Computed field type cannot be inferred")
                maybeExprType
    ...
    and resolveComputedField
        (stack: Set<ResolvedFieldRef>)
        (entity: HalfResolvedEntity)
        (fieldRef: ResolvedFieldRef)
        (comp: HalfResolvedComputedField)
        : PossiblyBroken<ResolvedComputedField> =
        ...
        | None ->
            ...
            | None ->
                ...
                try
                    ...
                    field
                with :? ResolveLayoutException as e when comp.Source.AllowBroken || forceAllowBroken ->
                    Error
                        { Error = e
                          AllowBroken = comp.Source.AllowBroken }

    and resolveLocalExpr
        (entityRef: ResolvedEntityRef)
        (expr: ParsedFieldExpr)
        : SingleFieldExprInfo * ResolvedFieldExpr =
    ...
    let resolveCheckConstraint
        (entityRef: ResolvedEntityRef)
        (constrName: ConstraintName)
        (constr: SourceCheckConstraint)
        : ResolvedCheckConstraint =
        let (exprInfo, expr) =
            resolveRelatedExpr initialWrappedLayout entityRef (parseRelatedExpr constr.Expression)
    ...
    let resolveIndexColumn
        (entityRef: ResolvedEntityRef)
        (indexType: IndexType)
        (colExpr: string)
        : ResolvedIndexColumn =
        let col =
            match parse tokenizeOzmaQL indexColumn colExpr with
            | Ok r -> r
    ...
    let resolveIndex (entityRef: ResolvedEntityRef) (indexName: IndexName) (index: SourceIndex) : ResolvedIndex =
        ...
        let includedExprs =
            Array.map (fun x -> resolveLocalExpr entityRef (parseRelatedExpr x) |> snd) index.IncludedExpressions

        let predicate =
            Option.map (fun x -> resolveLocalExpr entityRef (parseRelatedExpr x) |> snd) index.Predicate
    ...
    let rec checkSaveRestoreKey
        (visited: Set<ResolvedEntityRef>)
        (entityRef: ResolvedEntityRef)
        (entity: HalfResolvedEntity)
        (saveRestoreKey: ConstraintName)
        =
        if Map.containsKey entityRef cachedSaveRestoredEntities then
            ...
            let constr =
                ...
                | None ->
                    raisef
                        ResolveLayoutException
                        "Alternate key %O specified as save-restore key not found"
                        saveRestoreKey
                | Some constr when not constr.IsAlternateKey ->
                    raisef
                        ResolveLayoutException
                        "Unique constraint %O specified as save-restore key is not an alternate key"
                        saveRestoreKey
                | Some constr -> constr
            ...
            for KeyValue(fieldName, field) in entity.ColumnFields do
                ...
                ignore
                <| checkSaveRestoreField keyColumn visited { Entity = entityRef; Name = fieldName } field
            ...
            for KeyValue(childRef, child) in entity.Children do
                ...
                if Option.isSome childEntity.SaveRestoreKey then
                    raisef
                        ResolveLayoutException
                        "Child entity %O can't have save-restore key declared, as it conflicts with definition in parent entity %O"
                        childRef
                        entityRef
                ...
                for KeyValue(fieldName, field) in childEntity.ColumnFields do
                    if Option.isNone field.InheritedFrom then
                        ignore
                        <| checkSaveRestoreField true visited { Entity = childRef; Name = fieldName } field

            cachedSaveRestoredEntities <- Map.add entityRef parent cachedSaveRestoredEntities

    and checkSaveRestoreField
        (keyColumn: bool)
        (visited: Set<ResolvedEntityRef>)
        (fieldRef: ResolvedFieldRef)
        ...
        | FTScalar(SFTReference(refEntityRef, opts)) ->
            if Set.contains refEntityRef visited then
                if keyColumn then
                    raisef
                        ResolveLayoutException
                        "Recursive reference to %O in save-restore key column %O is forbidden"
                        refEntityRef
                        fieldRef
                // We don't support recursion in custom save-restored entity values for now.
                    ...
                    raisef
                        ResolveLayoutException
                        "Recursive reference to %O in save-restored column %O is not yet supported"
                        refEntityRef
                        fieldRef
            else
                ...
                | None ->
                    raisef
                        ResolveLayoutException
                        "Entity %O referenced in %O is not save-restored"
                        refEntityRef
                        fieldRef
        ...
        | FTScalar(SFTEnum _) -> ()
        | typ when keyColumn ->
            raisef ResolveLayoutException "Type %O of field %O is not supported for key columns" typ fieldRef
        | _ -> ()
    ...
    let resolveEntity (entityRef: ResolvedEntityRef) (entity: HalfResolvedEntity) : ResolvedEntity =
        ...
        let mapCheckConstraint name constr =
            ...
            with e ->
                raisefWithInner ResolveLayoutException e "In check constraint %O" name

        let checkConstraints = Map.map mapCheckConstraint entity.Source.CheckConstraints

        let checkConstraintNames =
            let makeName (name, c: ResolvedCheckConstraint) =
                (checkConstraintSQLName entity.HashName c.HashName, { Entity = entityRef; Name = name })
            ...
            with Failure msg ->
                raisef
                    ResolveLayoutException
                    "Check constraint names clash (first %i characters): %s"
                    hashNameLength
                    msg
        ...
        let mapIndex name index =
            ...
            with e ->
                raisefWithInner ResolveLayoutException e "In index %O" name

        let indexes = Map.map mapIndex entity.Source.Indexes

        let indexNames =
            let makeName (name, i: ResolvedIndex) =
                (indexSQLName entity.HashName i.HashName, { Entity = entityRef; Name = name })

            try
                indexes |> Map.toSeq |> Seq.map makeName |> Map.ofSeqUnique
            with Failure msg ->
                raisef ResolveLayoutException "Index names clash (first %i characters): %s" hashNameLength msg
    ...
    let resolveSchema (schemaName: SchemaName) (schema: SourceSchema) : ResolvedSchema =
        let mapEntity name entity =
            ...
            with e ->
                raisefWithInner ResolveLayoutException e "In entity %O" name
        ...
        try
            entities
            |> Map.values
            |> Seq.map (fun ent -> ent.HashName)
            |> Set.ofSeqUnique
            |> ignore
        with Failure msg ->
            raisef ResolveLayoutException "Entity names hash clash: %s" msg
    ...
    let resolveLayout () : Layout =
        ...
        let saveRestoredEntities =
            sortSaveRestoredEntities (Map.toSeq cachedSaveRestoredEntities) |> Seq.toArray
...
let private fillInternallyDeletedEntities
    (layout: Layout)
    (referencesMap: ReferencingEntitiesMap)
    (initialInternallyDeletedEntities: Set<ResolvedEntityRef>)
    : Set<ResolvedEntityRef> =
...
type private Phase3Resolver(layout: Layout, internallyDeletedEntities: Set<ResolvedEntityRef>) =
    let mutable cachedComputedFields: Map<ResolvedFieldRef, ResolvedComputedField> =
        Map.empty
        ...
        if not isRoot then
            ...
            let getCaseType (case: VirtualFieldCase, currComp: ResolvedComputedField) =
                if currComp.IsMaterialized <> comp.IsMaterialized then
                    raisef
                        ResolveLayoutException
                        "Virtual computed fields cannot be partially materialized: %O"
                        fieldRef
            ...
            let caseTypes =
                computedFieldCases layout ObjectMap.empty fieldRef comp |> Seq.map getCaseType
    ...
    let rec resolveComputedField (fieldRef: ResolvedFieldRef) (comp: ResolvedComputedField) : ResolvedComputedField =
        ...
        | Some parentRef ->
            ...
            let ret =
                resolveComputedField origFieldRef (Map.find fieldRef.Name origEntity.ComputedFields |> Result.get)
...
let resolveLayout (forceAllowBroken: bool) (layout: SourceLayout) : Layout =
    ...
    let referencingFields =
        fillReferencingFields phase1.RootEntities entities phase1.ReferencingFields

    let cascadeDeletedEntities =
        fillCascadeDeletedEntities entities phase1.CascadeDeletedEntities
    ...
    let internallyDeletedEntities =
        fillInternallyDeletedEntities layout2 referencingFields phase1.InternallyDeletedEntities
