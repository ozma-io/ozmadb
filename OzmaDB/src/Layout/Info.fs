...
type SerializedChildEntity =
    { Ref: ResolvedEntityRef; Direct: bool }
...
let serializeComputedField (comp: ResolvedComputedField) : SerializedComputedField =
    { Expression = comp.Expression.ToOzmaQLString()
...
let private checkUpdateTriggers (name: FieldName) (triggers: MergedTriggersTime) =
    Map.containsKey MUFAll triggers.OnUpdateFields
    || Map.containsKey (MUFField name) triggers.OnUpdateFields

let serializeColumnField
    (triggers: MergedTriggersEntity)
    (name: FieldName)
    (column: ResolvedColumnField)
    : SerializedColumnField =
    { FieldType = column.FieldType
      ...
      HasUpdateTriggers =
        checkUpdateTriggers name triggers.Before
        || checkUpdateTriggers name triggers.After }
...
let serializeIndex (index: ResolvedIndex) : SerializedIndex =
    { Expressions = index.Expressions |> Array.map serializeIndexColumn
      ...
      Predicate = index.Predicate |> Option.map (fun x -> x.ToOzmaQLString())
      Type = index.Type }
...
let serializeEntity (layout: Layout) (triggers: MergedTriggersEntity) (entity: ResolvedEntity) : SerializedEntity =
    { ColumnFields = Map.map (serializeColumnField triggers) entity.ColumnFields
      ComputedFields =
        Map.mapMaybe
            (fun name col -> col |> Result.getOption |> Option.map serializeComputedField)
      ...
      Children =
        entity.Children
        |> Map.toSeq
        |> Seq.map (fun (ref, info) -> { Ref = ref; Direct = info.Direct })
        |> Seq.toArray
      Root = entity.Root
...
let serializeSchema
    (layout: Layout)
    (triggersSchema: MergedTriggersSchema)
    ...
    let mapOne name entity =
        if entity.IsHidden then
            ...
            let triggers =
                Map.findWithDefault name emptyMergedTriggersEntity triggersSchema.Entities
...
let serializeLayout (triggers: MergedTriggers) (layout: Layout) : SerializedLayout =
    let mapOne name schema =
        let triggersSchema =
            Map.findWithDefault name emptyMergedTriggersSchema triggers.Schemas
