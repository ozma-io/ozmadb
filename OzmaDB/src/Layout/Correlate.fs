...
let private correlateSchemaMeta
    (schemaName: SchemaName)
    (schemaKeys: MigrationKeysSet, schema: SchemaMeta)
    : MigrationKeysSet * SchemaMeta =
                ...
                | Some idName ->
                    let correlateIdColumn name (keys, column: ColumnMeta) =
                        if name = idName then
                            ...
                            | CTPlain { DefaultExpr = Some(VEFunc(SQLName "nextval",
                                                                  [| VEValue(VRegclass { Schema = idSeqSchema
                                                                                         Name = idSeqName }) |])) } when
                                idSeqSchema = Some schemaName
                                || (schemaName = SQLName "public" && idSeqSchema = None)
                                ->
...
let correlateDatabaseMeta (meta: DatabaseMeta) : DatabaseMeta =
    { meta with
        Schemas = Map.map correlateSchemaMeta meta.Schemas }
