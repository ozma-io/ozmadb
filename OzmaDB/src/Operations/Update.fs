...
let private getDbSet<'a when 'a :> DbContext> (defaultSchema : SchemaName) (db : 'a) (property : PropertyInfo) =
    ...
    if not (typ.IsGenericType && typ.GetGenericTypeDefinition() = typedefof<DbSet<_>>) then
        ...
        let ref = { Schema = schema; Name = OzmaQLName tableName }
        Some (entityType, ref)
...
let private schemaEntityRef : ResolvedEntityRef = { Schema = funSchema; Name = OzmaQLName "schemas" }
...
let private makeEntity schemaName (entity : Entity) = ({ Schema = schemaName; Name = OzmaQLName entity.Name }, entity)
let private makeSchema (schema : Schema) = schema.Entities |> Seq.ofObj |> Seq.map (makeEntity (OzmaQLName schema.Name))
