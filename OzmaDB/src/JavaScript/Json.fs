...
type private PendingArray = { Array: IList<obj> }
...
and V8JsonWriter(engine: V8JsonEngine) =
    ...
    let pushValue (value: obj) =
        if stack.Count = 0 then
            ...
            | :? PendingArray as array -> array.Array.Add(value)
    ...
    override this.WriteEndObject() =
        ...
        | (true, (:? PendingObject as obj)) ->
            match obj.PropertyName with
            | Some propName ->
                raisef JsonWriterException "Expected value for property \"%s\" before object end" propName
            | None ->
                stack.Pop() |> ignore
                pushValue obj.Object
        | _ -> raisef JsonWriterException "Expected object start before object end"

    override this.WriteComment(text) =
        raisef JsonWriterException "Cannot write comment"

    override this.WriteStartConstructor(name) =
        raisef JsonWriterException "Cannot write constructor"

    override this.WriteEndConstructor() =
        raisef JsonWriterException "Cannot write constructor"

    override this.WriteRaw(json) =
        raisef JsonWriterException "Cannot write raw JSON"

    override this.WriteRawValue(json) =
        raisef JsonWriterException "Cannot write raw JSON"

    override this.WriteStartArray() =
        base.WriteStartArray()
        let arr = { Array = engine.CreateArray() }: PendingArray
        stack.Push(arr)
    ...
    override this.WritePropertyName(name: string) =
        ...
        | (true, (:? PendingObject as obj)) ->
            match obj.PropertyName with
            | Some propName ->
                raisef JsonWriterException "Expected value for property \"%s\" before property name" propName
    ...
    override this.WriteValue(value: byte[]) : unit =
        raisef JsonWriterException "Cannot write byte array"

    override this.WriteValue(value: Guid) =
        base.WriteValue(value)
        pushValue <| string value

    override this.WriteValue(value: TimeSpan) : unit =
        raisef JsonWriterException "Cannot write time span"
...
type V8JsonReader(rootValue: obj) =
    ...
    member private this.Unpack(value: obj) =
        match value with
        | :? char as c -> this.SetToken(JsonToken.String, string c)
        | :? byte as i -> this.SetToken(JsonToken.Integer, box i)
        | :? int16 as i -> this.SetToken(JsonToken.Integer, box i)
        | :? uint16 as i -> this.SetToken(JsonToken.Integer, box i)
        | :? int32 as i -> this.SetToken(JsonToken.Integer, box i)
        | :? uint32 as i -> this.SetToken(JsonToken.Integer, box i)
        | :? int64 as i -> this.SetToken(JsonToken.Integer, box i)
        | :? uint64 as i -> this.SetToken(JsonToken.Integer, box i)
        | :? single as number ->
        ...
        | :? decimal as number ->
            ...
            if number = Int128.op_Explicit integer then
                ...
                this.SetToken(JsonToken.Float, box number)
        | :? bool as boolVal -> this.SetToken(JsonToken.Boolean, box boolVal)
        | :? string as str -> this.SetToken(JsonToken.String, str)
        | null -> this.SetToken(JsonToken.Null)
        | :? Undefined -> this.SetToken(JsonToken.Undefined)
        | :? IList<obj> as array ->
            stack.Push(array.GetEnumerator())
            this.SetToken(JsonToken.StartArray)
        | :? DateTime as date -> this.SetToken(JsonToken.Date, box date)
        | :? IDictionary<string, obj> as obj ->
            stack.Push(obj.GetEnumerator())
            this.SetToken(JsonToken.StartObject)
        | _ -> raisef JsonReaderException "Unsupported value data type: %O" (value.GetType())
