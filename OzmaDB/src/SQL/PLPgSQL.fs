...
type RaiseMessage =
    ...
    member this.ToPLPgSQLString() =
        let args =
            this.Options
            |> Seq.map (fun o -> sprintf ", %s" (o.ToSQLString()))
            |> String.concat ""
    ...
    member this.ToPLPgSQLString() =
        ...
        let str =
            ...
            | StIfThenElse(ifs, els) ->
                ...
                let firstStr =
                    sprintf "IF %s THEN %s" (firstCase.ToSQLString()) (stmtsToString firstStmts)

                let otherCases =
                    ifs
                    |> Seq.tail
                    |> Seq.map (fun (case, stmts) ->
                        sprintf "ELSIF %s THEN %s" (case.ToSQLString()) (stmtsToString stmts))
                    |> String.concat " "
        ...
        let optionsStr =
            if Map.isEmpty this.Options then
                ...
                let opts =
                    this.Options
                    |> Map.toSeq
                    |> Seq.map (fun (opt, v) -> sprintf "%s = %s" (opt.ToPLPgSQLString()) (v.ToSQLString()))
                    |> String.concat ", "
        ...
        let declarations =
            if Array.isEmpty this.Declarations then
                ...
                sprintf
                    "DECLARE %s"
                    (this.Declarations
                     |> Seq.map (fun x -> x.ToPLPgSQLString())
                     |> String.concat "; ")
